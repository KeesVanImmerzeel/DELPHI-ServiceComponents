{: Contains the Date & Time Routines used by
 ESB Professional Computation Suite.

 This is designed to work in Delphi 4 and above.<p>

 There is a fair bit of overlap between our Freeware ESBDates and
 this unit. However routines here take better advantage of the
 features offered in Delphi 4 and above.<p>

 For routines involving Time Zones see: <See unit=ESBPCSTimeZones>.
...
{: A Speed Optimised Routine to get the Current Date. Time Portion is Zero.
 @cat DTMath
 @deprecated Please use <See Routine=ESBToday> instead
}
function OptDate: TDateTime;
{$IFDEF D6AndAbove}deprecated;
{$ENDIF}
...
{: Returns the Long Day of Weeks Description for the supplied Date. Relies
 on Regional Settings.
 @param DT Date/Time to convert.
...
begin
     try
          ...
          begin
               ...
               try
                    Result := Int2EStr (Hrs) + FormatSettings.TimeSeparator
                         + Int2ZStr (Mins, 2);
               finally
...
begin
     ...
     begin
          case Length (S) of
               4: S := LeftStr (S, 2) + FormatSettings.TimeSeparator + RightStr (S, 2);
               6: S := LeftStr (S, 2) + FormatSettings.TimeSeparator + Copy (S, 3, 2)
                    + FormatSettings.TimeSeparator + Copy (S, 5, 2);
          end;
     end;
...
begin
     ...
     DateOrder := GetESBDateOrder (FormatSettings.ShortDateFormat);
     ...
     try
          ...
          begin
               case Length (S) of
                    4: S := LeftStr (S, 2) + FormatSettings.DateSeparator + RightStr (S, 2);
                    6: S := LeftStr (S, 2) + FormatSettings.DateSeparator + Copy (S, 3, 2)
                         + FormatSettings.DateSeparator + Copy (S, 5, 2);
                    8:
                         begin
                              if DateOrder = edoYMD then
                                   S := LeftStr (S, 4) + FormatSettings.DateSeparator + Copy (S, 5, 2)
                                        + FormatSettings.DateSeparator + Copy (S, 7, 2)
                              else
                                   S := LeftStr (S, 2) + FormatSettings.DateSeparator + Copy (S, 3, 2)
                                        + FormatSettings.DateSeparator + Copy (S, 5, 4);
                         end;
               end;
          end
          else
          begin
               ...
               for I := 1 to 12 do
               begin
                    P1 := Pos (UpperCase (FormatSettings.LongMonthNames [I]), S);
                    if P1 > 0 then
                    begin
                         S := LeftStr (S, P1 - 1) + Int2EStr (I) +
                              RightAfterStr (S, P1 + Length (FormatSettings.LongMonthNames [I]) - 1);
                         Found := True;
                         Break;
                    end;
               end;
               ...
               begin
                    for I := 1 to 12 do
                    begin
                         P1 := Pos (UpperCase (FormatSettings.ShortMonthNames [I]), S);
                         if P1 > 0 then
                         begin
                              S := LeftStr (S, P1 - 1) + Int2EStr (I) +
                                   RightAfterStr (S, P1 + Length (FormatSettings.ShortMonthNames [I]) - 1);
                              Break;
                         end;
                    end;
               end;
          ...
          try
               ...
               repeat
                    P1 := Pos (FormatSettings.DateSeparator + FormatSettings.DateSeparator, S);
                    if P1 <> 0 then
                         Delete (S, P1, 1);
               until P1 = 0;

               P1 := ESBPosCh (FormatSettings.DateSeparator, S);
               if P1 > 0 then // If at least one Date Separator
               begin
                    P2 := ESBPosCh (FormatSettings.DateSeparator, Copy (S, P1 + 1, Length (S) - P1));
                    ...
                    begin
                         ...
                         case DateOrder of
                              edoDMY, edoMDY:
                                   begin
                                        S := S + FormatSettings.DateSeparator + Int2EStr (ThisYear)
                                   end;
                              edoYMD:
                                   begin
                                        S := Int2EStr (ThisYear) + FormatSettings.DateSeparator + S;
                                   end;
                         end;
                    end;
               end
               ...
               begin
                    ...
                    case DateOrder of
                         edoDMY:
                              begin
                                   S := S + FormatSettings.DateSeparator + Int2EStr (ThisMonth)
                                        + FormatSettings.DateSeparator + Int2EStr (ThisYear);
                              end;
                         edoMDY:
                              begin
                                   S := Int2EStr (ThisMonth) + FormatSettings.DateSeparator + S
                                        + FormatSettings.DateSeparator + Int2EStr (ThisYear);
                              end;
                         edoYMD:
                              begin
                                   S := Int2EStr (ThisYear) + FormatSettings.DateSeparator +
                                        Int2EStr (ThisMonth) + FormatSettings.DateSeparator + S;
                              end;
                    end;
               end;
          ...
          try
               ...
               begin
                    ...
                    begin
                         ...
                         case DateOrder of
                              edoDMY:
                                   begin
                                        Mnth := Str2Int (RightAfterChStr (S, FormatSettings.DateSeparator));
                                   end;
                         else
                              Mnth := Str2Int (LeftTillChStr (S, FormatSettings.DateSeparator));
                         end;
                         ...
                         case DateOrder of
                              edoDMY, edoMDY:
                                   begin
                                        S := S + FormatSettings.DateSeparator + Int2EStr (Yr)
                                   end;
                              edoYMD:
                                   begin
                                        S := Int2EStr (Yr) + FormatSettings.DateSeparator + S;
                                   end;
                         end;
                    end;
               end
               ...
               begin
                    ...
                    case DateOrder of
                         edoDMY:
                              begin
                                   S := S + FormatSettings.DateSeparator + Int2EStr (Mnth)
                                        + FormatSettings.DateSeparator + Int2EStr (Yr);
                              end;
                         edoMDY:
                              begin
                                   S := Int2EStr (Mnth) + FormatSettings.DateSeparator + S
                                        + FormatSettings.DateSeparator + Int2EStr (Yr);
                              end;
                         edoYMD:
                              begin
                                   S := Int2EStr (Yr) + FormatSettings.DateSeparator +
                                        Int2EStr (Mnth) + FormatSettings.DateSeparator + S;
                              end;
                    end;
               end;
...
var
     Hold: string;
begin
     Hold := FormatSettings.ShortDateFormat;
     FormatSettings.ShortDateFormat := Format;
     try
          Result := Str2Date (DateStr);
     finally
          FormatSettings.ShortDateFormat := Hold;
     end;
end;
...
begin
     ...
     Result := FormatSettings.ShortMonthNames [Date2Month (DT)] + ' ' + Int2ZStr (Year mod 100, 2);
end;
...
begin
     Year := OptDate2Year (DT);
     Result := FormatSettings.ShortMonthNames [Date2Month (DT)] + ' ' + Int2ZStr (Year, 4);
end;
...
begin
     ...
     for I := 1 to 12 do
          if S = AnsiUpperCase (FormatSettings.ShortMonthNames [I]) then
          begin
               Result := True;
               Break
          end;
...
begin
     ...
     S := AnsiUpperCase (Month);
     for I := 1 to 12 do
          if S = AnsiUpperCase (FormatSettings.LongMonthNames [I]) then
          begin
               Result := True;
               Break
          end;
...
begin
     ...
     S := AnsiUpperCase (DOW);
     for I := 1 to 12 do
          if S = AnsiUpperCase (FormatSettings.ShortDayNames [I]) then
          begin
               Result := True;
               Break
          end;
...
begin
     ...
     S := AnsiUpperCase (DOW);
     for I := 1 to 12 do
          if S = AnsiUpperCase (FormatSettings.LongDayNames [I]) then
          begin
               Result := True;
               Break
          end;
