{: Contains the Data Type Conversions and String Routines used by
...
uses
     Windows,
     ESBPCS_RS_Globals, ESBPCS_RS_Math,
     ESBPCSMath, ESBPCSSystem;
...
function StripThousandSeparators (const S: string): string;
begin
     Result := StripChFromStr (S, FormatSettings.ThousandSeparator);
end;
...
begin
     ...
     try
          ...
          Temp := '';
          for I := 1 to L2 do
               Temp := FormatSettings.ThousandSeparator + Copy (Result, LS - 3 * I + 1, 3) + Temp;
...
begin
     try
          ...
          begin
               repeat
                    Found := False;
                    if S2 [Length (S2)] in ['e', 'E', FormatSettings.DecimalSeparator, '-', '+'] then
                    begin
