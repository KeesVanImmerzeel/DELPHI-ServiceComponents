{: Contains the Numeric Edit Components for ESB Professional
 Computation Suite.
 ...
 For General Edit Components see <See Unit=ESBPCSEdit>. For Date/Time
 Edit Components see <See Unit=ESBPCSDateTimeEdit>. For Currency Edit
 Components see <See Unit=ESBPCSCurrEdit>.<p>
...
uses
     ...
     ESBPCSGlobals, ESBPCSGlobals2, ESBPCSEdit;
...
type
     ...
     private
          ...
          fShowThousandSeparators: Boolean; // Should Thousand Separators be used
     ...
     protected
          ...
          procedure SetShowThousandSeparators (Value: Boolean);
     ...
     published
          ...
          property ThousandSeparators: TESBSeparatorStyle
               ...
               default essNone;
          {: When enabled, the Edit Box will display the Value using
           Thousand Separators as defined in your Regional Settings.
           @deprecated This property is maintained for Backward compatibility,
           ThousandSeparators is the property that should be used.
           }
          property ShowThousandSeparators: Boolean
               read fShowThousandSeparators
               write SetShowThousandSeparators
               stored False;
   ...
   Set to 1 for no Scale to by Applied. }
          ...
          {: Font Color for the field when it contains a non-Negative Value
           ...
           Font.Color Property is ignored. }
          property ColorFontPos;
          {: When enabled and SciRange is enabled and the value is
           not displayed in Scientific Notation, then the Edit Box will
           display the Value using  Thousand Separators as defined in your Regional Settings.
           }
          property ShowThousandSeparators: Boolean
               read fShowThousandSeparators
               write SetShowThousandSeparators
...
uses
     SysUtils, Windows,
     ESBPCS_RS_Math,
     ESBPCSConvert, ESBPCSMath, ESBPCSMsgs;
...
begin
     ...
     fShowThousandSeparators := False;
     fCustomSeparators := False;
     fCustomDecimal := FormatSettings.DecimalSeparator;
     fCustomThousands := FormatSettings.ThousandSeparator;
end;
...
begin
     ...
     if not (Key in [VK_RETURN, VK_Tab, VK_Prior, VK_Next, VK_UP, VK_DOWN]) and (fThousandSeparators = essAuto) then
     begin
...
begin
     HoldCh1 := FormatSettings.ThousandSeparator;
     HoldCh2 := FormatSettings.DecimalSeparator;
     try
          if fCustomSeparators then
          begin
               FormatSettings.ThousandSeparator := fCustomThousands;
               FormatSettings.DecimalSeparator := fCustomDecimal
          end;
          ...
          P := ESBPosCh (FormatSettings.DecimalSeparator, S);
          if P = 0 then
               P := Length (S) + 1;
          Cnt := 0;
          ...
          begin
               ...
               if (Cnt = 4) then
               begin
                    S := LeftStr (S, I) + FormatSettings.ThousandSeparator + RightAfterStr (S, I);
                    Cnt := 1;
               end;
          end;
          Text := S;
          ...
          if fCustomSeparators then
          begin
               FormatSettings.ThousandSeparator := HoldCh1;
               FormatSettings.DecimalSeparator := HoldCh2
          end;
     end;
end;
...
begin
     ...
     try
          ...
          begin
               ...
               else if (Key = FormatSettings.DecimalSeparator) then // Handle '.' pressing
               begin
                    if Null then
                    begin
                         Null := False;
                    ...
                    if ESBPosCh (FormatSettings.DecimalSeparator, Text) > 0 then // See if there is already a Decimal Separator
               ...
               begin
                    P := ESBPosCh (FormatSettings.DecimalSeparator, Text);
...
procedure TESBBaseNumericEdit.SetShowThousandSeparators (Value: Boolean);
begin
     if fShowThousandSeparators <> Value then
     begin
          fShowThousandSeparators := Value;
          if fShowThousandSeparators then
               fThousandSeparators := essExit
          else
               fThousandSeparators := essNone;
          DisplayText;
     end;
...
begin
     ...
     begin
          ...
          fShowThousandSeparators := (fThousandSeparators <> essNone);
          DisplayText;
     end;
end;
...
begin
     if CustomSeparators then
          ...
          FValidChar := ['+', FormatSettings.ThousandSeparator, '0'..'9'];
...
begin
     ...
     try
          ...
          begin
               HoldCh := FormatSettings.ThousandSeparator;
               try
                    if CustomSeparators then
                         FormatSettings.ThousandSeparator := CustomThousands;
                    S := Int2CEStr (fIntValue);
               finally
                    if CustomSeparators then
                         FormatSettings.ThousandSeparator := HoldCh;
               end;
          end
          else
...
begin
     if CustomSeparators then
          ...
          FValidChar := ['+', FormatSettings.ThousandSeparator, FormatSettings.DecimalSeparator, '0'..'9'];
...
begin
     ...
     begin
          FormatSettings.ThousandSeparator := CustomThousands;
          FormatSettings.DecimalSeparator := CustomDecimal;
     end;
     try
          if fThousandSeparators <> essNone then // See if ThousandSeparator need to be used.
          begin
...
begin
     inherited Create (AOwner);
     FValidChar := ['+', '-', FormatSettings.ThousandSeparator, FormatSettings.DecimalSeparator, '0'..'9', 'E', 'e'];
     FValidCharHold := FValidChar;
     fSciRange := False;
     fSciMin := 0.0001;
     fSciMax := 1000000.0;
...
procedure TESBSciFloatEdit.SetShowThousandSeparators (Value: Boolean);
begin
     if fShowThousandSeparators <> Value then
     begin
          fShowThousandSeparators := Value;
